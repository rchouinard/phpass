 ...
 *
 * The SHA-256 Crypt algorithm was designed to replace MD5 Crypt by Ulrich
 * Drepper in 2008. SHA-256 Crypt is recommended for new applications.
 *
 * Supported parameters:
 *
 * <ul>
 *   <li><b>rounds:</b> Optional number of rounds to use. Must be an integer
 *   between 1000 and 999999999 inclusive. Defaults to 80000.</li>
 *
 ...
 * @license MIT License - http://www.opensource.org/licenses/mit-license.php
 */
class SHA256Crypt implements Hash
{
    ...
    public static function genHash($password, $config)
    {
        ...
        if (!preg_match('/^\$5\$(?:rounds=\d{4,9}\$)?[\.\/0-9A-Za-z]{0,16}\$[\.\/0-9A-Za-z]{43}$/', $hash)) {
            $hash = ($config == '*0') ? '*1' : '*0';
        }
